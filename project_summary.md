...
- `function`: **`count_beans(acc)`** (Dòng 24)
  > *Mô tả:* Đếm tổng số đậu thần trong balo.
- `function`: **`has_giftcode_items(acc)`** (Dòng 33)
  > *Mô tả:* Kiểm tra có item giftcode trong balo không.
- `function`: **`count_bua_items(acc)`** (Dòng 41)
  > *Mô tả:* Đếm số loại bùa đã có trong balo.
- `function`: **`count_set_lien_hoan(acc)`** (Dòng 50)
...
- `async_function`: **`_activate_item_2000(acc, ctrl, log_func)`** (Dòng 62)
...
- `async_function`: **`buy_bua(acc, log_func)`** (Dòng 17)
...
- `async_function`: **`select_character(acc, log_func)`** (Dòng 98)
  > *Mô tả:* Chọn nhân vật mặc định và vào game.
Đợi cho đến khi map_id > 0 và c_power > 0.

------------------------------

### 📄 File: `commands/setup/step_equip_master.py`
- `async_function`: **`equip_master(acc, log_func, C)`** (Dòng 16)
  > *Mô tả:* Mặc đồ cho sư phụ: tìm bản sao đã ép của mỗi item và equip lên body.
Chỉ equip items đã ép (có option 102 hoặc 95/96).

------------------------------

### 📄 File: `commands/setup/step_equip_pet.py`
- `async_function`: **`equip_pet(acc, log_func, C)`** (Dòng 17)
  > *Mô tả:* Mặc đồ cho đệ tử: tìm bản sao đã ép của mỗi item và đưa cho đệ tử.
Chỉ đưa items đã ép.
...
- `async_function`: **`wait_giftcode_response(acc, ctrl, log_func, timeout: float)`** (Dòng 20)
  > *Mô tả:* Tương thích ngược: gọi GiftcodeService.wait_response.
- `async_function`: **`check_giftcode_items(acc, log_func)`** (Dòng 28)
  > *Mô tả:* Tương thích ngược: gọi GiftcodeService.check_reward_items.
- `async_function`: **`submit_giftcode(acc, npc_id: int, code: str, log_func)`** (Dòng 34)
  > *Mô tả:* Tương thích ngược: gọi GiftcodeService.submit_giftcode.
Returns True nếu thành công hoặc đã dùng.
- `async_function`: **`_submit_giftcode_santa(acc, code: str, log_func)`** (Dòng 43)
  > *Mô tả:* Tương thích ngược: gọi GiftcodeService.submit_giftcode với Santa fallback.

------------------------------

### 📄 File: `commands/setup/step_rewards.py`
- `async_function`: **`open_muri(acc, log_func)`** (Dòng 18)
  > *Mô tả:* Teleport tới NPC Ông Muri và mở menu.
- `async_function`: **`claim_rewards(acc, state_mgr, log_func, force: bool)`** (Dòng 49)
...
- `async_function`: **`_claim_gold(acc, log_func)`** (Dòng 129)
  > *Mô tả:* Nhận vàng từ NPC nhà.
- `async_function`: **`_claim_gem(acc, npc_id, log_func)`** (Dòng 193)
  > *Mô tả:* Nhận ngọc xanh miễn phí từ NPC nhà.
- `async_function`: **`_claim_disciple(acc, npc_id, log_func)`** (Dòng 229)
  > *Mô tả:* Nhận đệ tử miễn phí từ NPC nhà.

------------------------------

### 📄 File: `commands/setup/step_santa_shop.py`
- `async_function`: **`santa_shop(acc, log_func)`** (Dòng 18)
...
- `async_function`: **`_upgrade_one_by_one(svc, acc, log_func, C, item_id, copy_idx, target_stars, mat_id, mat_qty, max_total)`** (Dòng 27)
  > *Mô tả:* Ép từng cái một, kiểm tra sao sau mỗi lần, retry nếu cần.
...
- `async_function`: **`upgrade_item_16(acc, log_func, C)`** (Dòng 112)
  > *Mô tả:* Ép sao Item 1 (trang bị) bằng Item 16 (đá pha lê).
Server EpSaoTrangBi.java yêu cầu đúng 2 items: 1 trangBi + 1 daPhaLe.
Mỗi lần combine: 1x Item 1 (equipment) + 1x Item 16 (crystal).
- `async_function`: **`_rada_combine_one(svc, acc, log_func, C, copy_idx, mat_id, mat_label, use_item_upgrade)`** (Dòng 189)
  > *Mô tả:* Ép 1 lần cho rada với material 442 hoặc 441.
Đảm bảo tab combine luôn mở trước khi gửi lệnh.

Args:
    use_item_upgrade: True=dùng open_item_upgrade(), False=dùng open_combine()
- `async_function`: **`upgrade_other_items(acc, log_func, C)`** (Dòng 244)
  > *Mô tả:* Ép trang bị (items 1, 7, 22, 28, 12):
...
- **Class `SetupAccountsCommand`** (Dòng 67)
  > *Mô tả:* Lệnh tự động setup tài khoản mới — dựa trên state machine.
  - `method`: **`__init__(manager)`** (Dòng 70)
  - `method`: **`_log(acc, msg: str)`** (Dòng 78)
  - `method`: **`_step_log(acc, step: int, msg: str)`** (Dòng 84)
  - `async_method`: **`execute(*args, **kwargs)`** (Dòng 90)
  - `async_method`: **`_setup_one(acc, idx, end_idx, force, start_step)`** (Dòng 177)
  - `async_method`: **`_run_step(acc, step: int, force: bool)`** (Dòng 241)
    > *Chi tiết:* Dispatch tới step handler tương ứng.
  - `async_method`: **`_ensure_logged_in(acc, C)`** (Dòng 279)
  - `async_method`: **`_wait_game_ready(acc, C, timeout)`** (Dòng 294)
...
Sub-types:
  ...
  6 = OPEN_ITEM
  - `async_method`: **`eat_pea()`** (Dòng 411)
    > *Chi tiết:* Tìm và ăn đậu trong hành trang khi HP/MP thấp.
  - `method`: **`find_item_in_bag(item_id: int)`** (Dòng 415)
    > *Chi tiết:* Tìm item trong hành trang và trả về danh sách kết quả.
  - `async_method`: **`use_item_by_id(item_id: int, action_type: int)`** (Dòng 419)
    > *Chi tiết:* Tìm và thực hiện hành động với item theo ID.
  - `async_method`: **`attack_nearest_mob()`** (Dòng 423)
    > *Chi tiết:* Tấn công quái vật gần nhất một lần.
  - `async_method`: **`auto_upgrade_stats(target_hp: int, target_mp: int, target_sd: int)`** (Dòng 427)
...
- **Class `CharacterHandler`** (Dòng 9)
  ...
  - `method`: **`process_power_info(msg: Message)`** (Dòng 261)
    > *Chi tiết:* Xử lý thông tin sức mạnh (POWER_INFO -115).
  - `method`: **`process_player_up_exp(msg: Message)`** (Dòng 274)
    > *Chi tiết:* Xử lý cập nhật EXP/Sức mạnh theo loại gói tin PLAYER_UP_EXP.
  - `method`: **`process_item_buy(msg: Message)`** (Dòng 294)
    > *Chi tiết:* Xử lý cập nhật tài sản khi mua/bán item (Cmd 6).
  - `method`: **`process_me_change_coin(msg: Message)`** (Dòng 308)
    > *Chi tiết:* Xử lý thay đổi xu từ server (Cmd -1).
  - `method`: **`process_me_up_coin_bag(msg: Message)`** (Dòng 318)
...
- **Class `InventoryHandler`** (Dòng 10)
  ...
  - `async_method`: **`eat_pea()`** (Dòng 245)
    > *Chi tiết:* Tìm và ăn đậu trong hành trang khi HP/MP thấp.
  - `method`: **`find_item_in_bag(item_id: int)`** (Dòng 280)
    > *Chi tiết:* Tìm item trong hành trang và trả về danh sách kết quả.
  - `async_method`: **`use_item_by_id(item_id: int, action_type: int)`** (Dòng 292)
...
- Chi phí dùng capsule: 1.5 (Ưu tiên hơn đi bộ 2 map, nhưng kém hơn đi bộ 1 map)
  ...
  - `async_method`: **`handle_walk_move(next_map: NextMap)`** (Dòng 910)
...
### 📄 File: `scratch/inspect_bag.py`
- `async_function`: **`main()`** (Dòng 14)

------------------------------

### 📄 File: `scratch/search_log.py`
- *(File cấu hình hoặc file rỗng, không chứa hàm)*
...
### 📄 File: `services/combine_service.py`
- **Class `CombineService`** (Dòng 83)
  > *Mô tả:* Dịch vụ combine / ép sao / đập đồ — tái sử dụng cho mọi module.
  - `method`: **`__init__(acc, log_func: Optional[Callable])`** (Dòng 86)
    > *Chi tiết:* Args:
    acc: Account object
    log_func: Hàm log(msg) tùy chọn. Nếu None, dùng print mặc định.
  - `method`: **`count_item(item_id: int)`** (Dòng 103)
    > *Chi tiết:* Đếm item trong balo (delegate tới InventoryService).
  - `async_method`: **`refresh_inventory()`** (Dòng 107)
    > *Chi tiết:* Refresh inventory (delegate tới InventoryService).
  - `method`: **`find_menu_option(options: list[str], *keywords)`** (Dòng 111)
    > *Chi tiết:* Tìm tùy chọn menu (delegate tới NavigationService).
  - `method`: **`_get_item_stars(item)`** (Dòng 119)
    > *Chi tiết:* Đọc số sao ép sao từ item object.
...
Chỉ option 102 mới là ép sao star count.
  - `method`: **`_get_star_count(item_id: int)`** (Dòng 132)
    > *Chi tiết:* Lấy số sao ép sao hiện tại của item (chỉ option 102).
  - `method`: **`_get_star_count_max(item_id: int)`** (Dòng 141)
    > *Chi tiết:* Lấy số sao ép sao cao nhất (chỉ option 102).
  - `method`: **`get_item_stars(item_id: int)`** (Dòng 152)
    > *Chi tiết:* Lấy số sao cao nhất của item trong balo.
  - `method`: **`get_item_star_at_index(bag_index: int)`** (Dòng 156)
    > *Chi tiết:* Lấy số sao tại index cụ thể trong balo.
  - `method`: **`count_item(item_id: int)`** (Dòng 163)
    > *Chi tiết:* Đếm số lượng item trong balo.
  - `method`: **`dump_item_options(item_id: int)`** (Dòng 167)
    > *Chi tiết:* Debug: in tất cả option_ids và params của items trong balo.
  - `method`: **`get_item_details(item_id: int)`** (Dòng 177)
    > *Chi tiết:* Lấy chi tiết tất cả item theo ID trong balo.
Returns: [{"index": idx, "qty": quantity, "stars": sao,
            "info": "tên", "options": [...]}]
  - `method`: **`get_slot_max(item)`** (Dòng 197)
    > *Chi tiết:* Đọc số slot tối đa (option 107) của item — dùng để phân biệt ép sao vs pha lê hóa.
  - `method`: **`check_item_slots(item_id: int, target_stars: int)`** (Dòng 208)
    > *Chi tiết:* Kiểm tra chi tiết từng item trong balo theo ID.
Returns: {
    "total": tổng_số_cái,
    "filled": đã_có_sao,
    "empty": chưa_có_sao,
  ...
  - `method`: **`is_fully_upgraded(item_id: int, target_stars: int)`** (Dòng 249)
    > *Chi tiết:* Kiểm tra item đã đủ sao chưa (mọi item cùng loại đều đủ).
  - `method`: **`print_status(item_ids: list[int], target_stars: int)`** (Dòng 257)
    > *Chi tiết:* In trạng thái chi tiết từng trang bị.
  - `async_method`: **`open_combine(npc_id: int, map_id: int, menu_item_keywords: Optional[list[str]], submenu_keywords: Optional[list[str]])`** (Dòng 282)
    > *Chi tiết:* Mở tab combine tại NPC.
...
Returns: True nếu mở tab combine thành công
  - `async_method`: **`open_item_upgrade(npc_id: int, map_id: int)`** (Dòng 391)
    > *Chi tiết:* Mở tab nâng cấp vật phẩm (dùng cho rada với item 441/442).
...
Returns: True nếu mở tab thành công
  - `async_method`: **`do_combine(main_item_id: int, materials: list[tuple[int, int]], max_times: int, npc_id: int, bag_index: int)`** (Dòng 523)
    > *Chi tiết:* Thực hiện combine lặp lại: main + materials → xác nhận → verify.
...
Returns: Số lần thành công
  - `method`: **`find_item_indices(item_id: int, quantity: int)`** (Dòng 696)
    > *Chi tiết:* Tìm indices của item trong balo.
  - `async_method`: **`send_and_confirm(indices: list[int], npc_id: int)`** (Dòng 706)
    > *Chi tiết:* Gửi items lên combine và đợi kết quả.
Returns: 'success' | 'fail' | 'timeout' | ''
- `function`: **`detect_star_option(acc, item_id: int)`** (Dòng 30)
  > *Mô tả:* Tự động detect option_id cho sao pha lê.
Server dùng:
  - Option 102: ép sao (param = star count)
  - Option 107: pha lê hóa hoặc max slots
- `function`: **`is_item_upgraded(item)`** (Dòng 48)
  > *Mô tả:* Kiểm tra item đã được ép/chế tạo chưa.
...
- `function`: **`is_item_fully_upgraded(item)`** (Dòng 70)
  > *Mô tả:* Kiểm tra item đã được ép FULL chưa (đầy đủ các sao/chỉ số cần thiết).
...
### 📄 File: `services/giftcode_service.py`
- **Class `GiftcodeService`** (Dòng 31)
  > *Mô tả:* Dịch vụ nhập giftcode — tái sử dụng cho mọi module.
  - `method`: **`__init__(acc, log_func: Optional[Callable])`** (Dòng 34)
    > *Chi tiết:* Args:
    acc: Account object
    log_func: Hàm log(msg) tùy chọn
  - `method`: **`_ensure_server_event()`** (Dòng 51)
    > *Chi tiết:* Đảm bảo server_message_event tồn tại và là asyncio.Event().
Controller có thể bị re-init (ví dụ khi reconnect), làm mất event cũ.
  - `async_method`: **`wait_response(timeout: float)`** (Dòng 64)
    > *Chi tiết:* Đợi phản hồi server sau khi gửi giftcode.
...
Returns:
  ...
  - `async_method`: **`check_reward_items()`** (Dòng 116)
    > *Chi tiết:* Kiểm tra item 1680 trong inventory sau khi nhận giftcode.
  - `async_method`: **`submit_giftcode(code: str, npc_id: Optional[int], santa_map: Optional[int])`** (Dòng 130)
    > *Chi tiết:* Nhập giftcode tại NPC (tự động fallback tới Santa).
...
Returns:
  ...
  - `async_method`: **`_submit_at_npc(code: str, npc_id: int)`** (Dòng 153)
    > *Chi tiết:* Nhập giftcode tại NPC cụ thể.
  - `async_method`: **`_submit_at_santa(code: str, santa_map: Optional[int])`** (Dòng 183)
    > *Chi tiết:* Nhập giftcode tại Santa.
  - `method`: **`_log_giftcode_result(code: str, status: str, detail: str)`** (Dòng 222)
    > *Chi tiết:* Ghi log kết quả giftcode.

------------------------------

### 📄 File: `services/inventory.py`
- **Class `InventoryService`** (Dòng 88)
  > *Mô tả:* Dịch vụ thao tác với hành trang — tái sử dụng cho mọi module.
  - `method`: **`__init__(acc, log_func: Optional[Callable])`** (Dòng 91)
    > *Chi tiết:* Args:
    acc: Account object
    log_func: Hàm log(msg) tùy chọn
  - `method`: **`count_item(item_id: int)`** (Dòng 104)
    > *Chi tiết:* Đếm số lượng item theo ID trong balo.
  - `method`: **`find_item_index(item_id: int)`** (Dòng 108)
    > *Chi tiết:* Tìm index đầu tiên của item trong balo. Trả về -1 nếu không tìm thấy.
  - `method`: **`find_item_indices(item_id: int, quantity: int)`** (Dòng 112)
    > *Chi tiết:* Tìm nhiều index của item trong balo.
  - `method`: **`has_item(item_id: int)`** (Dòng 116)
    > *Chi tiết:* Kiểm tra có item trong balo không.
  - `method`: **`count_items_by_ids(item_ids: list[int])`** (Dòng 120)
    > *Chi tiết:* Đếm số lượng nhiều item IDs cùng lúc. Trả về {item_id: count}.
  - `async_method`: **`refresh()`** (Dòng 128)
    > *Chi tiết:* Refresh lại thông tin inventory từ server.
- `function`: **`count_item(acc, item_id: int)`** (Dòng 27)
  > *Mô tả:* Đếm số lượng item theo ID trong balo (free function).
- `function`: **`find_item_index(acc, item_id: int)`** (Dòng 36)
  > *Mô tả:* Tìm index đầu tiên của item trong balo. Trả về -1 nếu không.
- `function`: **`find_item_indices(acc, item_id: int, quantity: int)`** (Dòng 44)
  > *Mô tả:* Tìm nhiều index của item trong balo.
- `function`: **`has_item(acc, item_id: int)`** (Dòng 55)
  > *Mô tả:* Kiểm tra có item trong balo không.
- `function`: **`count_items_by_ids(acc, item_ids: list[int])`** (Dòng 63)
  > *Mô tả:* Đếm số lượng nhiều item IDs cùng lúc. Trả về {item_id: count}.
- `async_function`: **`refresh_inventory(acc)`** (Dòng 72)
  > *Mô tả:* Refresh lại thông tin inventory từ server (free function).
...
### 📄 File: `services/navigation.py`
- **Class `NavigationService`** (Dòng 204)
  > *Mô tả:* Dịch vụ di chuyển và tương tác NPC / menu — tái sử dụng cho mọi module.
  - `method`: **`__init__(acc, log_func: Optional[Callable])`** (Dòng 207)
    > *Chi tiết:* Args:
    acc: Account object
    log_func: Hàm log(msg) tùy chọn. Nếu None, dùng print mặc định.
  - `method`: **`find_npc(npc_id: int)`** (Dòng 222)
    > *Chi tiết:* Tìm NPC theo template_id trong bản đồ hiện tại.
  - `method`: **`find_npc_by_name(name: str)`** (Dòng 226)
    > *Chi tiết:* Tìm NPC theo tên (không phân biệt hoa thường).
  - `async_method`: **`teleport_to_npc(npc_id: int, y_offset: int)`** (Dòng 234)
    > *Chi tiết:* Teleport đến gần NPC.
  - `async_method`: **`open_menu(npc_id: int, timeout: float)`** (Dòng 242)
    > *Chi tiết:* Mở menu NPC, trả về danh sách tùy chọn.
  - `async_method`: **`confirm_menu(npc_id: int, option_idx: int, wait_next: bool, timeout: float)`** (Dòng 246)
    > *Chi tiết:* Xác nhận tùy chọn trong menu NPC.
  - `async_method`: **`open_input_form(npc_id: int, option_idx: int, timeout: float)`** (Dòng 251)
    > *Chi tiết:* Mở menu NPC → chọn option → đợi input form xuất hiện.
  - `method`: **`find_menu_option(options: list[str], *keywords)`** (Dòng 257)
    > *Chi tiết:* Tìm index tùy chọn trong menu chứa bất kỳ từ khóa nào. Trả về -1 nếu không.
  - `async_method`: **`go_home()`** (Dòng 265)
    > *Chi tiết:* Di chuyển về nhà theo giới tính nhân vật.
  - `async_method`: **`move_to_map(target_map_id: int)`** (Dòng 269)
    > *Chi tiết:* Di chuyển đến map theo ID bằng xmap.
- `function`: **`find_npc(acc, npc_id: int)`** (Dòng 38)
  > *Mô tả:* Tìm NPC theo template_id trong bản đồ hiện tại (free function).
- `function`: **`find_npc_by_name(acc, name: str)`** (Dòng 49)
  > *Mô tả:* Tìm NPC theo tên (không phân biệt hoa thường) (free function).
- `async_function`: **`teleport_to_npc(acc, npc_id: int, y_offset: int)`** (Dòng 59)
  > *Mô tả:* Teleport đến gần NPC (free function).
- `async_function`: **`open_menu_npc(acc, npc_id: int, timeout: float)`** (Dòng 77)
  > *Mô tả:* Mở menu NPC, trả về danh sách tùy chọn (free function).
- `async_function`: **`confirm_menu_npc(acc, npc_id: int, option_idx: int, wait_next: bool, timeout: float)`** (Dòng 90)
  > *Mô tả:* Xác nhận tùy chọn trong menu NPC (free function).
- `async_function`: **`open_input_form(acc, npc_id: int, option_idx: int, timeout: float)`** (Dòng 108)
  > *Mô tả:* Mở menu NPC → chọn option → đợi input form (free function).
- `function`: **`find_menu_option(options: list[str], *keywords)`** (Dòng 129)
  > *Mô tả:* Tìm index tùy chọn trong menu chứa bất kỳ từ khóa nào (free function).
- `async_function`: **`go_home(acc, log_func)`** (Dòng 139)
  > *Mô tả:* Di chuyển về nhà theo giới tính nhân vật (free function).
- `async_function`: **`move_to_map(acc, target_map_id: int, log_func)`** (Dòng 167)
  > *Mô tả:* Di chuyển đến map theo ID bằng xmap (free function).
...
### 📄 File: `services/retry.py`
- **Class `RetryConfig`** (Dòng 20)
  > *Mô tả:* Cấu hình retry cho mỗi thao tác.
  - `method`: **`__init__(max_attempts: int, base_delay: float, max_delay: float, backoff: float)`** (Dòng 23)
- `async_function`: **`run_with_timeout(coro: Awaitable, timeout: float, default_return)`** (Dòng 31)
  > *Mô tả:* Chạy coroutine với timeout, trả về default nếu timeout.
- `async_function`: **`retry_operation(acc, log_func, label: str, coro_factory: Callable, config: Optional[RetryConfig], timeout: float)`** (Dòng 40)
...
- **Class `ShowCommand`** (Dòng 9)
  - `method`: **`__init__()`** (Dòng 10)
  - `async_method`: **`execute(account: Account, *args, **kwargs)`** (Dòng 13)
  - `method`: **`_show_body_equip(account)`** (Dòng 261)
    > *Chi tiết:* Hiển thị trang bị sư phụ + đệ tử đang mặc (tổng hợp nhanh).
  - `method`: **`_show_body_items(items_list, prefix)`** (Dòng 293)
    > *Chi tiết:* Hiển thị chi tiết items trên body.
Args:
    items_list: list items (arr_item_body)
    prefix: tiền đề hiển thị ("Body" hoặc "PetBody")
  - `async_method`: **`_show_pet_equip(account)`** (Dòng 328)
    > *Chi tiết:* Hiển thị chi tiết trang bị đệ tử đang mặc.
...
- `function`: **`display_macro_help()`** (Dòng 111)
