        ...
        if cmd_base == "pet":
            ...
            if len(parts) > 1:
                ...
                if sub in ["on", "off"]:
                    ...
                    print(f"[{C.YELLOW}{account.username}{C.RESET}] Đã {'BẬT' if status else 'TẮT'} autoattack.")
                    ...
                    if len(parts) > 2:
                        ...
                        if target_arg == "nearest":
                            ...
                            if account.controller.auto_attack.set_target_nearest(target_type):
                                print(f"[{C.YELLOW}{account.username}{C.RESET}] {C.GREEN}Đã target {target_type} gần nhất{C.RESET}")
                            else:
                                print(f"[{C.YELLOW}{account.username}{C.RESET}] {C.RED}Không tìm thấy target{C.RESET}")
                        ...
                        # autoattack target mob <id>
                            ...
                            if len(parts) > 3 and parts[3].isdigit():
                                ...
                                if account.controller.auto_attack.set_target_mob(mob_id):
                                    print(f"[{C.YELLOW}{account.username}{C.RESET}] {C.GREEN}Đã target Mob {mob_id}{C.RESET}")
                                else:
                                    print(f"[{C.YELLOW}{account.username}{C.RESET}] {C.RED}Mob {mob_id} không tồn tại{C.RESET}")
                            else:
                                print(f"[{C.YELLOW}{account.username}{C.RESET}] Sử dụng: autoattack target mob <id>")
                        ...
                        # autoattack target char <id>
                            ...
                            if len(parts) > 3 and parts[3].isdigit():
                                ...
                                if account.controller.auto_attack.set_target_char(char_id):
                                    print(f"[{C.YELLOW}{account.username}{C.RESET}] {C.GREEN}Đã target Char {char_id}{C.RESET}")
                                else:
                                    print(f"[{C.YELLOW}{account.username}{C.RESET}] {C.RED}Char {char_id} không tồn tại{C.RESET}")
                            else:
                                print(f"[{C.YELLOW}{account.username}{C.RESET}] Sử dụng: autoattack target char <id>")
                        ...
                        # autoattack target name <tên> [mob|char|both]
                            ...
                            if len(parts) > 3:
                                ...
                                if account.controller.auto_attack.set_target_by_name(target_name, target_type):
                                    print(f"[{C.YELLOW}{account.username}{C.RESET}] {C.GREEN}Đã target '{target_name}'{C.RESET}")
                                else:
                                    print(f"[{C.YELLOW}{account.username}{C.RESET}] {C.RED}Không tìm thấy '{target_name}'{C.RESET}")
                            else:
                                print(f"[{C.YELLOW}{account.username}{C.RESET}] Sử dụng: autoattack target name <tên> [mob|char|both]")
                        
                        else:
                            print(f"[{C.YELLOW}{account.username}{C.RESET}] Sử dụng: autoattack target <nearest|mob|char|name>")
                    else:
                        print(f"[{C.YELLOW}{account.username}{C.RESET}] Sử dụng: autoattack target <nearest|mob|char|name> [args]")
                    ...
                    print(f"[{C.YELLOW}{account.username}{C.RESET}] Đã xóa target")
                
                else:
                    print(f"[{C.YELLOW}{account.username}{C.RESET}] Sử dụng: autoattack <on|off|target|clear>")
            else:
                print(f"[{C.YELLOW}{account.username}{C.RESET}] Sử dụng: autoattack <on|off|target|clear>")
